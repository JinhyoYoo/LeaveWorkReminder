...
story.append(P("v1.3  ·  macOS 14.0+ (Sonoma)  ·  Swift 6 / SwiftUI", s_cover_ver))
...
for i, t in enumerate([
    ...
    "알림 발송 → [확인] 누르면 모니터링 종료, [다음 버스 타자] 누르면 다음 버스 대기",
...
story.append(make_table(
    ...
    [
        ...
        ["버튼", "[확인] — 모니터링 중지 (오늘 알림 종료)\n[다음 버스 타자] — 현재 버스 스킵, 다음 버스 도착 시 재알림"],
    ],
    [20*mm, W - 20*mm]
))

story.append(Spacer(1, 3*mm))
story.append(P("알림 스타일은 Alert(사용자가 버튼을 누를 때까지 유지)로 설정해야 합니다. "
    "시스템 설정 → 알림 → 퇴근 알리미에서 알림 스타일을 '알림(Alerts)'으로 변경하세요. "
    "앱 메뉴바에서 '알림 설정 열기' 버튼으로 바로 이동할 수 있습니다.", s_note))
...
qa = [
    ...
    ("알림이 오지 않거나 바로 사라짐",
     ...
     "알림 스타일을 '알림(Alerts)'으로 설정하면 버튼을 누를 때까지 유지됩니다. "
     "앱 메뉴바의 '알림 설정 열기' 버튼으로 바로 이동 가능합니다. "
...
story.append(P("퇴근 알리미 v1.3  ·  macOS 14.0+  ·  2026", s_foot))
